...
export default function ShowPersonVisit({ isOpen, onClose, modalMode = "view", person = {}, document = {}, documents, mutate }) {
    ...
    const handleCheckBoxChange = (place) => {
        if(mode !== "view") {
            if (place === "Clase" && data.enviado === "Clase" || place === "Casa" && data.enviado === "Casa" || place === "Urgencias" && data.enviado === "Urgencias") {
                setData({
                    ...data,
                    "enviado": ''
                })
    ...
    const handleDeleteButton = () => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                const response = await post(`${URL}/deletePersonHistoryInfo`, {_id: data._id})
    ...
    return(
        ...
        <ModalOverlay />
        <ModalContent>
          <ModalHeader>{viewMode[mode].header} VISITA {person?.name.toUpperCase()} {person?.lastname.toUpperCase()}</ModalHeader>
          <ModalCloseButton />
          <ModalBody>
          </ModalBody>
            <Flex flexDirection='column' pl="6" pr="6">
                ...
                <Spacer/>
                    <Flex onClick={() => handleCheckBoxChange("Clase")}>
                        <Box w='25px' h='25px' borderWidth='1px' borderColor={softBlue} mr="4" bgColor={data.enviado === "Clase" ? (softBlue) : ('')}/>
                        <Text sx={regular12}>ENVIADO A CLASE</Text>
                    </Flex>
                    <Spacer/>
                    <Flex onClick={() => handleCheckBoxChange("Casa")}>
                        <Box w='25px' h='25px' borderWidth='1px' borderColor={softBlue} mr="4" bgColor={data.enviado === "Casa" ? (softBlue) : ('')}/>
                        <Text sx={regular12}>ENVIADO A CASA</Text>
                    </Flex>
                    <Spacer/>
                    <Flex onClick={() => handleCheckBoxChange("Urgencias")}>
                        <Box w='25px' h='25px' borderWidth='1px' borderColor={softBlue} mr="4" bgColor={data.enviado === "Urgencias" ? (softBlue) : ('')}/>
